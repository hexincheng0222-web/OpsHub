...
function handleImport(e: Event) {
  ...
  r.onload = async () => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (cols.length < EXPECTED_COLS) { skippedCols++; continue }
      rows.push({
    ...
    try {
      ...
      const msgs = [`导入 ${result.imported} 条`]
      if (skippedCols) msgs.push(`${skippedCols} 行列数不足(${EXPECTED_COLS}列)跳过`)
      if (result.errors?.length) msgs.push(`${result.errors.length} 条失败: ${result.errors[0]}`)
      msgs.length > 1 ? ElMessage.warning(msgs.join('，')) : ElMessage.success(msgs[0])
    } catch (e: any) {
